...
/**
 * Helper: Get Approvals Statistics
 */
  ...
  try {
    ...
    const [
      ...
      prisma.materialRequest
        ...
        .catch(() => 0),
      prisma.expense
        .count({ where: { projectId: { in: projectIds }, status: 'TODO' } })
        .catch(() => 0),
...
/**
 * Helper: Get Projects Statistics
 */
  ...
  try {
    ...
    const [
      ...
      prisma.project.count({
        where: { ...projectWhereClause, status: 'ONGOING' },
      }),
      prisma.project.count({
        where: { ...projectWhereClause, status: 'PLANNING' },
      }),
      prisma.project.count({
        where: { ...projectWhereClause, status: 'ON_HOLD' },
      }),
      prisma.project.count({
        where: { ...projectWhereClause, status: 'COMPLETED' },
      }),
      prisma.project.count({
        where: { ...projectWhereClause, status: 'CANCELLED' },
      }),
      prisma.project.count({
        where: { ...projectWhereClause, status: 'DELAYED' },
      }),
    ]);
